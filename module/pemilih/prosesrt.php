	...
	if($_SESSION['level']==""){
        ...
        // print_r($_POST);die();
        	if (isset($_POST['simpan'])) {
        		...
        		//Simpan ke tabel dpt
        		$query = mysqli_query($koneksi, "INSERT INTO pilihan_rt(id_user,id_dpt,no_urut,status)
        			VALUES ('$id_user','$id_dpt','$pilihan','$status')")
	                or die('Ada kesalahan pada query insert : '.mysqli_error($koneksi));

	                if ($query) {
             		// jika berhasil tampilkan pesan berhasil simpan data
                		$cek    = mysqli_num_rows(mysqli_query($koneksi,"SELECT id_user, status FROM pilihan_rw WHERE id_user='$id_user' AND status = 1"));
                              if ($cek > 0){
